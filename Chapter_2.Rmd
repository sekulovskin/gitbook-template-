...
```{r nice-fig, fig.cap='Plot of the 95% CI's!', out.width='80%', fig.asp=.75, fig.align='center'}
...
Here we will also try to illustrate the [Central Limit Theorem](https://en.wikipedia.org/wiki/Central_limit_theorem), with the most simple example.

First we draw 1000 samples (again of size 800), form a, say, *Poisson* distribution, of course we could've drawn them from a uniform or an exponential as well.
...
```{r nice-fig, fig.cap='Histogram of the sampling distribution of the mean!', out.width='80%', fig.asp=.75, fig.align='center'}
hist(t(means))
```
